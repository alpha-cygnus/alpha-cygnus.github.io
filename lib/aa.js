OO.newPackage('AA', function(OO, Tone, $) {
	...
	AA.newClass(AA.Base, function Device() {}, {
		...
		getJSON2: function(withName) {
			var res = [this._cls];
			var params = {};
			if (withName) params._name = this.name;
			for (var pn in (this.hashParam || {})) {
				var p = this.hashParam[pn];
				var v = p._get();
				if (p._cls == 'ParamFloat') {
	...
	AA.newClass(AA.Base, function Rack() {}, {
		...
		getJSON2: function() {
			...
			for (var i = 0; i < this.devSort.length; i++) {
				var dev = this.devSort[i].dev;
				rack.push(dev.getJSON2(true));
			};
			res.push(rack);
			var mxs = {'matrix-analog': this.matrixAnalog, 'matrix-digital': this.matrixGate};
			for (var k in mxs) {
				var mx = mxs[k];
				var ms = [k];
				for (code in mx.connects) {
					var v = mx.connects[code]._get();
					if (v) {
						ms.push([code.replace('>', '-'), Math.round(v*1000)/1000]);
					}
				}
				res.push(ms);
			}
	...
	AA.newClass(AA.Base, function Unit() {}, {
		_init: function() {
			this.matrixAnalog = AA.newObj(['Matrix', 'Analog', this]);
			this.matrixGate = AA.newObj(['Matrix', 'Gate', this]);
			// this.devList = {};
			...
			for (var i = 0; i < arguments.length; i++) {
				if ($.isArray(arg)) {
					if (arg[0] == 'rack' || arg[0] == 'devices') {
						for (var j = 1; j < arg.length; j++) {
							this.addDevice(arg[i]);
						}
					}
					if (arg[0] == 'matrix-analog' || arg[0] == 'analog-matrix') {
						for (var j = 1; j < arg.length; j++) {
							this.addDevice(arg[i]);
						}
					}
